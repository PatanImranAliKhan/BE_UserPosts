...
exports.followUser = async (req, res, next) => {
    try {
        ...
        if (id === userId) {
            return res.status(200).json({ Message: "The Id provided was same as your userId" })
        }
        const followerData = await checkFollowing(userId, id);
        if (!id || !(await verifyFollowingUser(id))) {
            return res.status(400).json({ Error: "ID was incorrect, provide proper UserId" })
        }
        let followingDetails = null;
        if (followerData && followerData.active == true) {
            return res.status(200).json({ Message: "You are already following the user with Id: "+id });
        }
        if (followerData) {
        ...
        return res.status(200).json({ Message: "Successfully followed UserId: "+id });
    } catch (e) {
        return res.status(500).json({ Error: "Something Error caught on Following user, please try after some time "+e })
    }
}
...
exports.unfollowUser = async (req, res, next) => {
    try {
        ...
        return res.status(200).json({ Message: `UnFollowed userId ${id} Successfully` });
    } catch (e) {
        return res.status(500).json({ Error: "Something Error caught on unFollowing user, please try after some time" })
    }
}